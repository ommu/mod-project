...
class ClientController extends Controller
{
	...
	public function accessRules() 
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('suggest','ajaxadd',
					'manage','add','edit','view','runaction','delete','publish'),
				'users'=>array('@'),
				'expression'=>'isset(Yii::app()->user->level) && (Yii::app()->user->level == 1)',
			),
	...
	public function actionAjaxAdd() 
	{
		...
		if(isset($_POST['client_name'])) {
			...
			if($model->save()) {
				...
				Yii::app()->session['project_client_id'] = $model->client_id;
			}
		}
	}
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['ProjectClient'])) {
			$model->attributes=$_POST['ProjectClient'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'id' => 'partial-project-client',
							'msg' => '<div class="errorSummary success"><strong>Project Client success created.</strong></div>',
						));
					} else {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-project-client',
							'msg' => '<div class="errorSummary success"><strong>Project Client success updated.</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				ProjectClient::model()->updateAll(array(
					'published' => 1,
				),$criteria);
				...
				ProjectClient::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-project-client',
						'msg' => '<div class="errorSummary success"><strong>Project Client success deleted.</strong></div>',
					));
				}
			}
			...
			$this->pageTitle = 'Delete Client: '.$model->client_name;
			$this->pageDescription = '';
			$this->pageMeta = '';
			...
			if(isset($id)) {
				...
				if($model->update()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-project-client',
						'msg' => '<div class="errorSummary success"><strong>ProjectClient success published.</strong></div>',
					));
				}
			}
	...
	{
		$model = ProjectClient::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='project-client-form') {
