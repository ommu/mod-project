...
class CategoryController extends Controller
{
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>Project Category success created.</strong></div>',
						));
					} else {
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['ProjectCategory'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('setting/index'),
							'id' => 'partial-project-category',
							'msg' => '<div class="errorSummary success"><strong>Project Category success updated.</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			$this->pageTitle = 'Update Category: '.$model->name;
			$this->pageDescription = '';
			$this->pageMeta = '';
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				ProjectCategory::model()->updateAll(array(
					'published' => 1,
				),$criteria);
				...
				ProjectCategory::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('setting/index'),
						'id' => 'partial-project-category',
						'msg' => '<div class="errorSummary success"><strong>Project Category success deleted.</strong></div>',
					));
				}
			}
			...
			$this->pageTitle = 'Delete Project Category';
			$this->pageDescription = '';
			$this->pageMeta = '';
